...
def plot_gyration(
    ...
    mean_r2, error_r2 = analytical_error(r2, weights)
    ...
    ax.fill_between(lengths, mean_r2 - error_r2, mean_r2 + error_r2,
                    alpha=0.3, color="C0", label="error")
    ...
    ax_right.plot(lengths, np.sum(
        alive[:, :max_step], axis=0), color='gray', alpha=0.5)
...
if __name__ == "__main__":
    ...
    plot_end_to_end(lengths, end_to_ends,
                    weights[:, :max_step], dimension)

    weighted_gyrations = np.sum(gyrations * weights[:, :max_step], axis=0) / np.sum(
        weights[:, :max_step], axis=0
    )
    plot_gyration(lengths, gyrations, weights[:, :max_step], dimension)
