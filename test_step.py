...
from polymer_code.simulate import (
    get_allowed_sides_2d,
    init_polymer_storage,
    do_step,
    perm_step,
)
...
def test_perm_step_enrichment_and_pruning():
    ...
    chains[3, step + 1] = [
        5.0,
        5.0,
    ]  # Marking to check if this chain was enriched later on
    ...
    assert (
        chains_new.shape[0] == num_chains + 1
    ), "Chains not duplicated properly during enrichment"
    ...
    assert (
        len(duplicate_indices) >= 2
    ), "Enrichment did not create a duplicate of chain 3"

    # Verify weights of duplicates are halved correctly
    for i in duplicate_indices:
        assert np.isclose(
            weights_new[i, step + 1], original_weight3[step + 1] / 2
    ...
    for i in range(alive_new.shape[0]):
        ...
        if len(false_indices) > 0:
            ...
            assert np.all(
                ~alive_new[i, first_false:]
            ), f"Chain {i} becomes alive after dying"
